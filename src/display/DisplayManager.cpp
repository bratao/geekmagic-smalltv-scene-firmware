 ...
 */
auto DisplayManager::playGifFullScreen(const String& path, uint32_t timeMs, bool keepScreen) -> bool {
    SceneRenderer::stop();
    if (g_gif == nullptr) {
        g_gif = new Gif();
        if (g_gif == nullptr) {
    ...
    if (!keepScreen) { DisplayManager::clearScreen(); }
 ...
 * @return true
 */
auto DisplayManager::stopGif(bool keepScreen) -> bool {
    if (g_gif != nullptr) {
        g_gif->stop();
    }
