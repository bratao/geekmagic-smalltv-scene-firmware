...
auto Gif::update() -> void {
    ...
    if (result <= 0) {
        if (m_loopEnabled && !m_stopRequested && !m_currentPath.isEmpty()) {
            ...
            // AnimatedGIF returns 0 after drawing the last frame, with its delay.
            // Reopening the stream must not discard that frame's display time.
            m_delayMsFromGif = delayMsFromGif;
            m_targetMs = (result == 0 && delayMsFromGif > 0)
                             ? static_cast<uint32_t>(delayMsFromGif) : GIF_FRAME_MS;
            m_lastFrameMs = now;
            m_startMs = millis();
            m_frameCount = 0;
