...
void registerApiEndpoints(Webserver* webserver) {
    ...
    webserver->raw().on("/api/v1/draw/clear", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
            return;
        }
        if (!validateDrawing(webserver, "clear")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawClear(webserver);
    });
    webserver->raw().on("/api/v1/draw/text", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
        ...
        if (!validateDrawing(webserver, "text")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawText(webserver);
    });
    webserver->raw().on("/api/v1/draw/rect", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
        ...
        if (!validateDrawing(webserver, "rect")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawRect(webserver);
    });
    webserver->raw().on("/api/v1/draw/circle", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
        ...
        if (!validateDrawing(webserver, "circle")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawCircle(webserver);
    });
    webserver->raw().on("/api/v1/draw/line", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
        ...
        if (!validateDrawing(webserver, "line")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawLine(webserver);
    });
    webserver->raw().on("/api/v1/draw/pixel", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
        ...
        if (!validateDrawing(webserver, "pixel")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawPixel(webserver);
    });
    webserver->raw().on("/api/v1/draw/triangle", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
        ...
        if (!validateDrawing(webserver, "triangle")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawTriangle(webserver);
    });
    webserver->raw().on("/api/v1/draw/ellipse", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
        ...
        if (!validateDrawing(webserver, "ellipse")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawEllipse(webserver);
    });
    webserver->raw().on("/api/v1/draw/roundrect", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
        ...
        if (!validateDrawing(webserver, "roundrect")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawRoundRect(webserver);
    });
    webserver->raw().on("/api/v1/draw/batch", HTTP_POST, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        if (webserver->raw().arg("plain").length() > 8192) {
            webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"body too large\"}");
        ...
        // Inspect the mode without changing active display state.
        JsonDocument sceneRequest;
        const auto parseError = deserializeJson(sceneRequest, webserver->raw().arg("plain"));
        if (!parseError && sceneRequest["mode"] == "scene") {
            if (webserver->raw().arg("plain").length() > 6144) {
                webserver->raw().send(413, "application/json", "{\"status\":\"error\",\"message\":\"scene body too large\"}");
                return;
            }
            String error;
            if (!sceneRequest.is<JsonObject>() || !SceneRenderer::submit(sceneRequest.as<JsonObjectConst>(), error)) {
                JsonDocument response;
                response["status"] = "error";
                response["message"] = error.isEmpty() ? "invalid scene" : error;
                String body; serializeJson(response, body);
                webserver->raw().send(400, "application/json", body);
                return;
            }
            webserver->raw().send(200, "application/json", "{\"status\":\"ok\",\"mode\":\"scene\"}");
            return;
        }
        sceneRequest.clear();
        if (!validateDrawing(webserver, "batch")) { return; }
        SceneRenderer::stop();
        DisplayManager::stopGif(true);
        handleDrawBatch(webserver);
    });
    webserver->raw().on("/api/v1/draw/status", HTTP_GET, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
        JsonDocument response;
        SceneRenderer::status(response.to<JsonObject>());
        String body; serializeJson(response, body);
        webserver->raw().send(200, "application/json", body);
    });
    webserver->raw().on("/api/v1/display/capabilities", HTTP_GET, [webserver]() {
        if (!requireBearerToken(webserver)) { return; }
        setCorsHeaders(webserver);
...
void handlePlayGif(Webserver* webserver) {
    ...
    const bool keepScreen = doc["keep_screen"] | false;
    bool playOk = DisplayManager::playGifFullScreen(foundPath, 0, keepScreen);
...
void handleStopGif(Webserver* webserver) {
    ...
    if (!body.isEmpty()) {
        if (deserializeJson(options, body)) {
            webserver->raw().send(400, "application/json", "{\"status\":\"error\",\"message\":\"invalid json\"}");
            return;
        }
    }
    const bool keepScreen = options["keep_screen"] | false;
    const bool stopped = DisplayManager::stopGif(keepScreen);
